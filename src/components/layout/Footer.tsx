...
const footerLinks = [
  ...
  },
  {
    heading: 'Products',
    links: [
      { label: 'Platinum Plus', href: '/products/platinum-plus' },
      { label: 'Jeevan Saathi', href: '/products/jeevan-saathi' },
      { label: 'Sehat Zindagi', href: '/products/sehat-zindagi' },
      { label: 'Takaful', href: '/products/takaful' },
      { label: 'Digital Insurance', href: '/products/digital-insurance' },
    ],
  },
  {
    heading: 'Services',
    links: [
      { label: 'Pay Premium', href: '/policyholder' },
      { label: 'Track Claim', href: '/policyholder' },
      { label: 'Find Agent', href: '/policyholder' },
    ],
  },
  {
    heading: 'Legal & Info',
    ...
    <footer role="contentinfo">
      ...
      <div className="bg-brand-500 py-10 md:py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h3 className="text-display-md text-white font-display">Download the State Life Health Plus App</h3>
          <div className="flex items-center justify-center gap-4 mt-6 flex-wrap">
            <a href="#" className="flex items-center gap-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl px-6 py-3 text-white transition-colors" aria-label="Download on the App Store">
            ...
            <a href="#" className="flex items-center gap-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl px-6 py-3 text-white transition-colors" aria-label="Get it on Google Play">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-10 lg:gap-8">
            <div className="sm:col-span-2">
              ...
              <p className="text-body-sm text-ink-500 mt-4 italic">"Ensuring Trust, Securing Futures"</p>
              ...
              <a href="tel:0800700900" className="flex items-center gap-2 text-brand-400 font-semibold text-body-sm mt-3">
                <Phone className="w-4 h-4" /> 0800-700-900
              </a>
              <div className="flex items-center gap-2 mt-5">
                {socialIcons.map(({ icon: Icon, href, label }) => (
                  <a key={label} href={href} className="w-9 h-9 rounded-lg bg-brand-800 hover:bg-brand-600 flex items-center justify-center text-white/70 hover:text-white transition-all duration-200" aria-label={label}>
            ...
            {footerLinks.map((group) => (
              <div key={group.heading}>
                <h4 className="text-body-xs text-ink-500 uppercase tracking-widest font-semibold pb-4 border-b border-brand-800 mb-4">{group.heading}</h4>
                <nav className="space-y-0.5">
                  {group.links.map((link) => (
                    <Link key={link.label} to={link.href} className="block text-body-sm text-ink-500 hover:text-white transition-colors py-1.5">
                      {link.label}
                    </Link>
                  ))}
                </nav>
          ...
          <div className="border-t border-brand-800 mt-12 pt-6 pb-4 flex items-center justify-between flex-wrap gap-4">
            <p className="text-body-xs text-ink-500">© {new Date().getFullYear()} State Life Insurance Corporation of Pakistan. All rights reserved.</p>
            <p className="text-body-xs text-ink-500">Regulated by SECP • Federal Insurance Ombudsman</p>
          </div>
        </div>
      </div>
    </footer>
